...
@interface FoodViewController2 ()
{
    int time;
}
@end

@implementation FoodViewController2
...
-(void)initChart:(NSArray *)arrData{
    ...
    NSMutableArray *arrQuantity = [[NSMutableArray alloc] init];
    for (ChartFoodRecord *item in arrData) {
        [arrDate addObject:[ShareHelper getDateTimeByFormatDay2:item.f_date]];
        [arrQuantity addObject:item.f_quantity];
    }
    ...
    // Line Chart No.1
    NSArray * data01Array = arrQuantity;
...
-(void)getData{
    ...
    [[ShareData instance].dataProxy getChartFood:data.a_id FromDate:fromDate ToDate:currentDate completionHandler:^(id result, NSString *errorCode, NSString *message) {
